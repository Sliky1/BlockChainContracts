...
function App() {
  ...
  const purchaseItem = async (id, price) => {
      const tx = await contract.purchaseItem(id, { value: ethers.parseEther(price) });
      await tx.wait();
      loadItems(contract);
            ...
            {items.map((item) => (
                <div key={item.id} className="item-card">
                    ...
                    {!item.isSold && item.owner.toLowerCase() !== account.toLowerCase() && (
                        <button className="button" onClick={() => purchaseItem(item.id, ethers.formatEther(item.price))}>
                            Purchase
                        </button>
                    )}
                </div>
